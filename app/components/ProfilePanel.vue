...
<div class="glass-jelly h-full w-full rounded-2xl flex flex-col ring-1 ring-fuchsia-500/10 dark:ring-pink-400/15">
  ...
  <div class="flex-1 overflow-y-auto">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-5 space-y-3 sm:space-y-4">
      ...
      <div
        class="bg-default/40 rounded-2xl sm:rounded-3xl p-5 sm:p-6 flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-6 border border-muted shadow-sm">
        ...
        <div class="flex flex-1 flex-col lg:flex-row lg:items-start lg:justify-between gap-5 lg:gap-8 min-w-0 w-full">
          <div class="flex-1 min-w-0 text-center sm:text-left space-y-3 w-full lg:pr-2">
            ...
            <div v-else class="space-y-3">
              <UInput v-model="editName" :placeholder="t('profile.display_name_placeholder')" class="w-full" size="lg"
            ...
            <div class="flex items-center justify-center sm:justify-start gap-2 pt-1">
              ...
              <template v-else>
                ...
                <button
                  ...
                  :disabled="saving || !editName.trim()" @click="saveProfile">
                  {{ saving ? t("profile.saving") : t("profile.save") }}
                </button>
              </template>
            </div>
            <div class="text-right">
              <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-primary/10 text-primary">{{ planName }}
                plan</span>
              <p v-if="planFeatureFlags.length" class="text-[11px] text-dimmed mt-1">
          ...
          <div
            ...
            <ClientOnly>
              <UColorModeSelect size="sm" variant="outline" color="neutral"
      ...
      <div class="bg-default/40 rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-muted shadow-sm space-y-4">
        <div class="flex items-center justify-between gap-2">
          <div class="flex items-center gap-2 min-w-0">
            ...
            <span class="text-sm font-bold text-default uppercase tracking-wide truncate">{{
              t("profile.usage_analytics")
            }}</span>
          </div>
        ...
        <div class="space-y-3">
          <div class="space-y-1.5">
            <div class="flex items-center justify-between text-sm gap-2">
              ...
              <span class="font-bold text-default tabular-nums">{{ usage.requestCount.value }} / {{ usage.limit.value
              }}</span>
            </div>
            <div
              class="h-2.5 w-full bg-black/10 dark:bg-white/10 rounded-full overflow-hidden ring-1 ring-black/5 dark:ring-white/5">
              <div class="h-full bg-primary shadow-[0_0_12px_rgba(168,85,247,0.4)] transition-all duration-700 ease-out"
      ...
      <div class="profile-masonry">
        <!-- Subscription -->
        <div
          class="profile-tile bg-default/40 rounded-2xl sm:rounded-3xl p-4 border border-muted shadow-sm flex flex-col gap-3">
          ...
          <button type="button"
            class="mt-0 w-full py-2 rounded-xl bg-primary border border-primary/20 text-white text-xs font-bold hover:shadow-[0_0_10px_rgba(168,85,247,0.3)] transition-all">
        ...
        <div class="profile-tile bg-default/40 rounded-2xl sm:rounded-3xl p-3 sm:p-4 border border-muted shadow-sm">
          <div class="flex items-center gap-1.5 mb-2">
            ...
            <span class="text-xs font-bold text-default uppercase tracking-wide">{{ t("profile.language_section")
            }}</span>
          </div>
          <p class="text-[10px] text-dimmed leading-snug mb-2">{{ t("profile.language_hint") }}</p>
          <div class="flex flex-wrap gap-1.5">
            <button v-for="l in locales" :key="typeof l === 'string' ? l : l.code" type="button"
              ...
              <UIcon v-if="(typeof l === 'string' ? l : l.code) === locale" name="i-lucide-check"
        ...
        <!-- Location -->
        <div v-if="userEmail"
          class="profile-tile bg-default/40 rounded-2xl sm:rounded-3xl p-4 border border-muted shadow-sm flex flex-col gap-2.5">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-2 min-w-0">
              ...
              <span class="text-sm font-bold text-default uppercase tracking-wide truncate">{{ t("profile.location")
              }}</span>
            </div>
            <ClientOnly>
          ...
          <div v-if="!config.public.serverUrl" class="pt-0.5">
            <div class="p-2 rounded-lg bg-amber-500/5 border border-amber-500/20 flex items-start gap-2">
              ...
              <p class="text-[10px] text-amber-600 dark:text-amber-400 leading-snug">{{ t("profile.backend_missing") }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          <button type="button" disabled
            class="w-full py-2 rounded-xl bg-default/80 text-muted text-xs font-bold opacity-50 cursor-not-allowed">
