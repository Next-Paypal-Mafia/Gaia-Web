...
<template>
<div
  class="glass-jelly flex flex-col h-full w-full overflow-hidden rounded-2xl relative ring-1 ring-fuchsia-500/10 dark:ring-pink-400/15">
  <!-- Coming Soon Overlay -->
  <div
    class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/70 dark:bg-black/60 backdrop-blur-md p-6 text-center">
    ...
    <p class="text-sm text-dimmed max-w-sm mb-6">
      We're building a secure, end-to-end encrypted vault to store your agent's confidential files, API keys, and
      documents.
    </p>
    <div
      class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-wider">
  ...
  <div class="flex flex-col h-full w-full opacity-40 pointer-events-none select-none">
    ...
    <div class="flex items-center gap-3 px-4 pt-4 pb-3 shrink-0">
      ...
      <button v-if="sidebarExpanded === false"
      ...
      <div
        class="flex-1 min-w-0 flex items-center gap-2 px-3 py-2 rounded-2xl bg-default/60 ring-1 ring-muted transition-colors"
      ...
      <div class="flex items-center gap-3 shrink-0">
        ...
        <div class="flex rounded-2xl bg-default/60 overflow-hidden ring-1 ring-muted">
        ...
        <!-- Add button -->
        <button
          class="flex items-center justify-center size-9 rounded-2xl bg-default/60 ring-1 ring-muted text-muted hover:text-default hover:bg-default/80 transition-colors"
          aria-label="Add">
        ...
        <UDropdownMenu :items="[
          ...
          <button
            class="flex items-center justify-center h-9 min-w-9 gap-1 px-2.5 rounded-2xl bg-default/60 ring-1 ring-muted text-muted hover:text-default hover:bg-default/80 transition-colors"
    ...
    <div class="flex-1 min-h-0 overflow-y-auto px-4 pb-4">
      ...
      <div v-if="viewMode === 'icon'" class="grid gap-4"
        ...
        <button v-for="file in filteredFiles" :key="file.id"
          ...
          <span class="text-[11px] text-default text-center line-clamp-2 wrap-break-word w-full px-0.5 leading-tight">
            {{ file.name }}
          </span>
        </button>
      </div>
      ...
      <div v-else class="flex flex-col divide-y divide-muted rounded-xl bg-default/30">
        <button v-for="file in filteredFiles" :key="file.id"
          class="flex items-center gap-3 px-4 py-2.5 hover:bg-default/60 transition-colors text-left w-full">
          <div class="size-9 rounded-lg flex items-center justify-center bg-default/60 shrink-0">
      ...
      <p v-if="filteredFiles.length === 0" class="py-8 text-center text-sm text-dimmed">
